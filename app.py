...
def debug_oauth():
    ...
    debug_info = {
        'GOOGLE_OAUTH_CLIENT_ID_exists': bool(client_id),
        'GOOGLE_OAUTH_CLIENT_SECRET_exists': bool(client_secret),
        'CLIENT_ID_length': len(client_id) if client_id else 0,
        'CLIENT_SECRET_length': len(client_secret) if client_secret else 0,
        'FLASK_DANCE_AVAILABLE': GOOGLE_OAUTH_AVAILABLE,
        'CLIENT_ID_preview': client_id[:20] + '...' if client_id else 'None',
        'CLIENT_SECRET_preview': client_secret[:20] + '...' if client_secret else 'None'
    }
...
if __name__ == "__main__":
    ...
    # For Windows compatibility, use threaded=True
    app.run(host='127.0.0.1', port=port, debug=debug_mode, threaded=True)

# Setup Google OAuth (must be outside main block for gunicorn)
with app.app_context():
    setup_google_oauth()
